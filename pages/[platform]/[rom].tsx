...
export async function getStaticPaths() {
  ...
  for (let i = 0; i < platforms.length; i++) {
    const filtered_roms = platforms[i].roms.filter(
      (r) => r.platformId === platforms[i].id,
    );
    for (let j = 0; j < filtered_roms.length; j++) {
      paths.push({
        params: { platform: platforms[i].url, rom: filtered_roms[j].url },
      });
    }
  }
...
const RomInfo = (props: any) => {
  return (
    <div className="flex flex-col border flex-wrap my-11">
      ...
      <div className="text-black bg-slate-50 border-y border-r border-sky-300 my-2">
        ...
        <span className="text-lg bg-slate-50 px-3 py-1">{props.rom.size}</span>
      </div>
            ...
            <div className="flex flex-col">
              ...
              <div className="grid md:grid-cols-2 border">
                <Image
                  ...
                  loading="lazy"
                />
                <div className="text-clip flex justify-center items-center">
                  Description:
                  <br />
                  {rom.game.description}
                </div>
              </div>
            </div>
            {/* <div>
